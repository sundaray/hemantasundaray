...
Now, you can import and use the `MotionH1` component inside your Server Component:
...
Now when `<HomePage>` renders, you'll see the enter animation on the `h1` element.

But h1 is not the only HTML element you'll wnat to animate. You might want to animate a div, a section, a p element etc. In such cases you'll have to create wrapper components like motionDiv, MotionSection etc. You can create a use-client.tsx file in your components folder, have wrapper components inside teh file and export them. you can then import these wrapper components to any Server Component you need and use for animation.
