...
We need to set up three different Supabase clients: two for authentication (client-side and server-side) and one for database interactions.

Create a new folder named `supabase` inside the `lib` folder, and inside `supabase`, create three files: `client.ts`, `client-db.ts` and `server.ts`.
...
In `client-db.ts`, add the following code:

```ts title="@/lib/supabase/client-db.ts"
...
Note that the Supabase database client can be used in both client and server components.
...
```tsx {6, 10-12, 22-23} title="@/app/layout.tsx"
